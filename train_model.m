function [net,stats] = train_model(net, imdb, getBatch, payload, varargin)
...
for epoch=start+1:opts.numEpochs
  ...
  if numel(opts.gpus) <= 1
    [stats.train(epoch),prof] = process_epoch(net, state, opts, 'train', payload) ;
    stats.val(epoch) = process_epoch(net, state, opts, 'val', payload) ;
    if opts.profile
      profview(0,prof) ;
      keyboard ;
    end
    ...
    spmd
      ...
      [stats_.train, prof_] = process_epoch(net_, state, opts, 'train', payload) ;
      stats_.val = process_epoch(net_, state, opts, 'val', payload) ;
...
% -------------------------------------------------------------------------
function [stats, prof] = process_epoch(net, state, opts, mode, payload)
% -------------------------------------------------------------------------
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    if strcmp(mode, 'train')
      ...
      net.mode = 'normal' ; % we make sure that the network of training is same to the network of validation
      net.eval(inputs) ;
      net.mode = 'test';
    end
  end
...
% -------------------------------------------------------------------------
function state = accumulate_gradients(state, net, opts, batchSize, mmap, payload)
% -------------------------------------------------------------------------
numGpus = numel(opts.gpus) ;
